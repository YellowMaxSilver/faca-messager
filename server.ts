...
async function createServer(){
    ...
    app.post('/api/addContactbyEmail',verifyUserExistence,async (req,res)=>{
        ...
        try{
            ...
            if(querySnapshot.empty){
                ...
                const docRef = await db.collection('contacts').add({ MainUid, [contactName]:contact});
                ...
                await db.collection('contacts').doc(docId).update({[contactName]:contactUid});
    ...
    app.post('/api/searchContacts',async (req,res)=>{
        ...
        try{
            const userRef = admin.firestore().collection('contacts');
            const querySnapshot = await userRef.where('MainUid','==',userId).get();
            if(!querySnapshot.empty){
                const result:any = [];
                querySnapshot.forEach((doc)=>{
                    result.push({id: doc.id, ...doc.data()});
